...
| Kontrol | Sonuç |
|---|---:|
| Brain IR v0.2 özel testleri | **22/22 geçti** |
| Tüm proje testleri (`node_modules` hariç) | **695/695 geçti** |
...
v0.1 ile kullanılan ölçüm tekrarlandı: 200 düğüm, 199 bağlantı ve 500 tik. Trace ve nedensellik eklenmiş ilk v0.2 sonucu 1602,97 ms idi. Gelen bağlantıların önceden indekslenmesi sonrasında ölçüm **1436,17 ms** oldu; son aksiyon beklenen şekilde `1` üretti. v0.1 ölçümü 156,27 ms idi.

Kalan maliyetin ana kaynağı trace ve causal metadata üretimidir. Gelen bağlantı indekslemesi düğüm×bağlantı taramasını kaldırdı ve ölçümü iyileştirdi. Büyük grafikler için yalnız değişen düğümlerin trace edilmesi ve trace depolamasının isteğe bağlı yapılması hâlâ gereklidir.
...
`inhibitor` düğümü artık doğrudan test edilmiştir. Bir input state'i negatif katkıya çevrilmiş ve hedef action trace'inde `inhibition` olarak görülmüştür.
