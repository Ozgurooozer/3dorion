// bridge/baglam.test.ts — bağlam sözleşmesinin iki kuralı.
"use strict";
import { test } from "node:test";
import assert from "node:assert/strict";
import { baglamMetni } from "./baglam.ts";
import type { BeyinGirdisi } from "./beyin.ts";

// Özet satırı `protocol/algi.ts` → `ozetle`nin ÜRETTİĞİ biçimdir (İngilizce
// çerçeve, Türkçe nesne adı — spec 06 §6.8). Elle uydurulmuş bir metin
// yazarsak test gerçek biçimi değil, hayalimizdekini sınar.
const girdi: BeyinGirdisi = {
  talimat: "T", sabit: "S", dunya: "D", ozetler: ["You looked (in front of you): yönetim terminali"],
  anilar: [], araclar: [],
  gecmis: [
    { rol: "kullanici", metin: "önünde ne var" },
    { rol: "orion", metin: "Bakıyorum." },
    { rol: "orion", metin: "Bakıyorum.", arac: true },
  ],
};

test("oturumlu beyinde geçmiş metne GİRMEZ — oturum zaten biliyor", () => {
  assert.doesNotMatch(baglamMetni(girdi).kullanici, /önünde ne var/);
});

test("durumsuz beyinde geçmiş girer, 'You looked' satırından ÖNCE ve tekrarsız", () => {
  const k = baglamMetni(girdi, { gecmis: true }).kullanici;
  assert.match(k, /Ozyn: önünde ne var/);
  assert.equal(k.match(/You: Bakıyorum\./g)?.length, 1, "aynı söz iki kez yazıldı");
  assert.ok(k.indexOf("Ozyn:") < k.indexOf("You looked"), "soru cevaptan sonra geldi");
});

test("satır sözleşmesi VARSAYILAN olarak girer — metinden ayrıştıran beyinler bozulmasın", () => {
  assert.match(baglamMetni(girdi).sistem, /KOMUT: </);
  assert.doesNotMatch(baglamMetni(girdi, { satirSozlesmesi: false }).sistem, /KOMUT: </);
});
