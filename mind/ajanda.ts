...
export interface AjandaAyari {
  /** İki öneri arasındaki asgari boşta süresi (ms). */
  asgariAralikMs?: number;
  /** Asgariye eklenen rastgele (ama determinist) uzatma üst sınırı (ms). */
  azamiSapmaMs?: number;
...
export class Ajanda {
  ...
  constructor(ayar: AjandaAyari = {}) {
    this._asgariMs = ayar.asgariAralikMs ?? 15_000;
    this._sapmaMs = ayar.azamiSapmaMs ?? 15_000;
